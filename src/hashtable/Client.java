...
public class Client {
  ...
  public static void main (String args[]) {
    ...
    while(clientNumber < 1 || clientNumber > 8 || !confirmed){
      System.out.println("Please enter you client number. (Between 1 and 8).");
      String number=sc.next();
      if(isInt(number)) {
        clientNumber=Integer.parseInt(number);
      }
      if(clientNumber >= 1 && clientNumber <=8 && isInt(number)) {
    ...
    server.start();
    Thread action = new Thread(new Action());
    action.start();
